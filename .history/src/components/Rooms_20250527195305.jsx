        ...
        <div className="space-y-4">
          {myRooms.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {myRooms.map((room) => (
                <button 
                  ...
                  className="w-full p-6 rounded-xl shadow-sm hover:shadow-md transition-all duration-300 text-left bg-gradient-to-br from-amber-50 to-amber-100 hover:from-amber-100 hover:to-amber-200 border-l-4 border-amber-400"
                >
                  <div className="flex items-start justify-between">
                    <h3 className="font-medium text-gray-900 mb-2 truncate max-w-[80%]">{room.name}</h3>
            ...
            <div className="flex flex-col items-center justify-center p-12 bg-white rounded-xl shadow-sm">
              ...
              <p className="text-gray-500 text-center">You haven't created any rooms yet</p>
            </div>
          )}
        </div>
        ...
        <div className="space-y-4">
          {addedRooms.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {addedRooms.map((room) => (
                <button 
                  ...
                  className="w-full p-6 rounded-xl shadow-sm hover:shadow-md transition-all duration-300 text-left bg-gradient-to-br from-emerald-50 to-emerald-100 hover:from-emerald-100 hover:to-emerald-200 border-l-4 border-emerald-400"
                >
                  <div className="flex items-start justify-between">
                    <h3 className="font-medium text-gray-900 mb-2 truncate max-w-[80%]">{room.name}</h3>
                  ...
                  <p className="text-sm text-gray-500 mt-2">You joined this room</p>
                </button>
              ))}
            </div>
          ) : (
            <div className="flex flex-col items-center justify-center p-12 bg-white rounded-xl shadow-sm">
              ...
              <p className="text-gray-500 text-center">You haven't joined any rooms yet</p>
            </div>
          )}
        </div>
      </div>
