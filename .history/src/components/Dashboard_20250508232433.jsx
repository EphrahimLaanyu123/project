...
const Dashboard = () => {
  ...
  useEffect(() => {
    async function fetchUserAndTasks() {
      ...
      const { data: taskAssignmentsData, error: taskAssignmentsError } = await supabase
      ...
      if (taskAssignmentsError) {
        ...
        setErrorMessage("Failed to fetch task assignments.");
        setIsLoading(false);
        return;
      }

      const taskIds = taskAssignmentsData?.map((assignment) => assignment.task_id) || [];
        ...
        <div className="p-6">
          ...
          <nav>
            <ul className="space-y-2">
              ...
              <li>
                <Link
                  to="/dashboard/tasks" // You might want a dedicated tasks page later
                  className="flex items-center px-4 py-3 text-gray-700 rounded-lg hover:bg-gray-100 transition-all duration-200 group"
                >
      ...
      <div className="flex flex-col flex-1 ml-72">
        ...
        <main className="flex-1 p-8 mt-[73px] overflow-y-auto bg-gradient-to-br from-gray-50 to-gray-100">
          <div className="max-w-7xl mx-auto">
            ...
            <Outlet /> {/* Render nested routes here, like the Rooms component */}
          </div>
        </main>
      </div>
    </div>
