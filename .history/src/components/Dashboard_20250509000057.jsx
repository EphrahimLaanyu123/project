import React, { Link, Outlet, useNavigate } from "react-router-dom";
...
const Dashboard = () => {
  ...
  const [assignedTasks, setAssignedTasks] = useState([]); // State to hold assigned tasks
  ...
  useEffect(() => {
    async function fetchUser() {
      ...
      const { data: taskData, error: taskError } = await supabase
        .from("task_members")
        .select("task_id, tasks(id, content, status, priority)") // Join with the tasks table
        .eq("user_id", userId);
